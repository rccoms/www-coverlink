...
const Registration = {
    ...
    initVehicle: function (user) {
        ...
        if (els.btnDelete) {
            els.btnDelete.onclick = () => {
                this.confirmWithModal('차량 번호를 삭제하시겠습니까?', async () => {
                    try {
                        const updated = await Auth.updateUser({ vehicleNumber: null });
                        if (updated) {
                        ...
                        console.error(e);
                        this.showAlertModal('삭제 중 오류가 발생했습니다.');
                    }
                });
            };
        }
        ...
        if (els.btnRegister) {
            els.btnRegister.onclick = async () => {
                ...
                try {
                    const updated = await Auth.updateUser({ vehicleNumber: val });
                    if (updated) {
                    ...
                    console.error(e);
                    this.showAlertModal('저장 중 오류가 발생했습니다.');
                }
            };
        }
    },
    ...
    initPhone: function (user) {
        ...
        if (els.btnDelete) {
            els.btnDelete.onclick = () => {
                this.confirmWithModal('휴대폰 번호를 삭제하시겠습니까?', async () => {
                    try {
                        const updated = await Auth.updateUser({ phoneNumber: null });
                        if (updated) {
        ...
        if (els.btnRegister) {
            els.btnRegister.onclick = async () => {
                ...
                try {
                    const updated = await Auth.updateUser({ phoneNumber: val });
                    if (updated) {
